...
const (
	...
	M_SUCCESS_CODES_EN = "success_codes_en"
	M_SUCCESS_CODES_ID = "success_codes_id"
)
...
func InitializeMaps(fileName string) error {
	...
	if err != nil {
		log.Errorf(logging.INTERNAL, "Error reading maps file %s", err.Error())
		return err
	}
	log.Info(logging.INTERNAL, "found map data file map_data.json")
	doc := MapsDocument{}
	if err := json.Unmarshal(bytes, &doc); err != nil {
		return err
	}
	for _, elem := range doc.Maps {
		log.Debugf(logging.INTERNAL, "populating map %s", elem.Name)
		maps[elem.Name] = elem.Entries
	}
	log.Info(logging.INTERNAL, "parsed map data file map_data.json")
...
func MustGetMap(mapName string) map[string]string {
	m, ok := maps[mapName]
	if !ok {
		//log.Criticalf(logging.INTERNAL, "missing map %s", mapName)
...
func MustGetString(mapName string, key string) string {
	...
	v, ok := m[key]
	if !ok {
		//log.Criticalf(logging.INTERNAL, "missing key %s in map %s", key, mapName)
